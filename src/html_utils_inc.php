...
function input_text($parameters, $echo = true)
{
  ...
  if(!empty($parameters["id"])) $html .= " id=\"" . escape_html($parameters["id"]) . "\"";
  ...
  if(!empty($parameters["value"])) $val = $parameters["value"];
  ...
  {
    if(in_array($name, $sys_fields)) continue;
...
function select($parameters, $echo = true)
{
  ...
  if(isset($parameters["multiple"])) $html .= " multiple";
  ...
  if(!empty($parameters["options"]))
  {
    ...
    else
    {
      foreach($parameters["options"] as $optval => $text)
      {
        ...
        if(!is_array($val) && $optval == $val) $selected = "selected";
        if(is_array($val) && in_array($optval, $val)) $selected = "selected";
...
function checkbox($parameters, $echo = true)
{
  ...
  if(!empty($parameters["value"])) $html .= " value=\"" . escape_html($parameters["value"]) . "\"";  
  ...
  if(!is_array($val) && !empty($val)) $checked = " checked";
  if(is_array($val) && in_array(checkempty($parameters["value"]), $val)) $checked = " checked";
  
  if(!empty($parameters["checked"])) $checked = " checked";
...
function radiobutton($parameters, $echo = true)
{
  ...
  if(!empty($val) && checkempty($parameters["value"]) == $val) $checked = " checked";
...
function table(&$array, $parameters = [], $echo = true)
{
  ...
  if(!empty($parameters["class"])) $html .= " class=\"" . escape_html($parameters["class"]) . "\"";  
  if(!empty($parameters["style"])) $html .= " style=\"" . escape_html($parameters["style"]) . "\"";  
  ...
  if(!empty($parameters["captions"]))
  {
    ...
    foreach($parameters["captions"] as $key => $value)
    {
      ...
      if(!empty($parameters["col_class_from_keys"])) $html .= " class=\"" . escape_html($key) . "\"";
      
      $html .= ">\n";
      
      if(empty($parameters["no_escape_html"])) $value = escape_html($value);

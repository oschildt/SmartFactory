  ...
  public function handleApiRequest()
  {
    ...
    foreach($handlers_to_call as $handler_key => $handler_class_name)    
    {
      if(!class_exists($handler_class_name)) 
      {
        ...
        $response_data["errors"] = [
          ["error_code" => "api_request_class_not_found", "error_text" => sprintf("The handler class '%s', defined for the request '%s', does not exist!", $handler_class_name, $handler_key)]
        ];
      ...
      if(!$handler_class->isSubclassOf("SmartFactory\Interfaces\IJsonApiRequestHandler"))
      {
        ...
        $response_data["errors"] = [
          ["error_code" => "api_request_wrong_class", "error_text" => sprintf("The handler class '%s', defined for the request '%s', does not implement the interface '%s'!", $handler_class_name, $handler_key, "IJsonApiRequestHandler")]
        ];
      ...
      if($handler_key == "#pre_process#" && !$this->do_standard_handling) break;
    } // foreach

...
{
  /**
   * Internal variable for storing the application root.
   *
   * @var string
   *
   * @author Oleg Schildt
  ...
  {
    if(!empty($parameters["app_root"])) $this->app_root = $parameters["app_root"];
    if(!empty($parameters["log_path"])) $this->log_path = $parameters["log_path"];
  ...
  protected function extract_call_stack($btrace)
  {
    ...
    foreach($btrace as $btrace_entry)
    {
      ...
      $trace .= str_replace($this->app_root, "", str_replace("\\", "/", $btrace_entry["file"]));

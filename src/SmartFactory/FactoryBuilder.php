...
class FactoryBuilder
{
  ...
  {
    if(empty($class)) throw new \Exception("Bound class is empty!");
    
    if(empty($interface_or_class)) throw new \Exception("Bound interface or class is empty!");

    if(!interface_exists($interface_or_class) &&
       !class_exists($interface_or_class)) throw new \Exception(sprintf("The interface or class '%s' does not exist!", $interface_or_class));

    if(!class_exists($class)) throw new \Exception(sprintf("The class '%s' does not exist!", $class));
    
    $ic = new \ReflectionClass($interface_or_class);
    $c = new \ReflectionClass($class);
    
    if(!$c->isInstantiable()) throw new \Exception(sprintf("The class '%s' is not instantiable!", $c->getName()));
    ...
    {
      if(!is_callable($init_function)) throw new \Exception(sprintf("'%s' is not a function!", $init_function));
  ...
  */
  ...
  {
    if(empty($interface_or_class)) throw new \Exception("Class or interface is not specified!");
    ...
    if(empty(self::$itable[$class_name])) throw new \Exception(sprintf("The interface or class '%s' has no bound class!", $class_name));

...
class MSSQL_DBWorker extends DBWorker
{
  ...
  {
    if(!empty($parameters["db_server"])) $this->db_server = $parameters["db_server"];
  ...
  public function close_connection()
  {
    ...
    if(is_resource($this->connection)) @sqlsrv_close($this->connection);
  ...
  public function fetch_row()
  {
    ...
    if(!is_resource($this->statement)) return false;
  ...
  public function field_info_by_num($num)
  {
    ...
    if(empty($info[$num])) return false;
    ...
    {
      if(in_array($field_info["type"], [-5, 3, 6, 4, 2, 7, 5, -6])) $field_info["numeric"] = 1;
      if(in_array($field_info["type"], [-2, -4, -3])) $field_info["binary"] = 1;

      if(isset($sqlsrv_type[$field_info["type"]])) $field_info["type"] = $sqlsrv_type[$field_info["type"]];
      else                                         $field_info["type"] = "undefined";
    }

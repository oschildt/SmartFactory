...
$active_tab = "texts";
if(!empty($_REQUEST["tab"])) $active_tab = $_REQUEST["tab"];
if(!in_array($active_tab, array_keys($tabs))) $active_tab = "texts";
...
function load_language_texts()
{
  ...
  {
    $messages .= "<p style='color:red; font-weight: bold'>Translation file 'localization/texts.xml' cannot be loaded!</p>";
    return false;
  }
  ...
  foreach($nodes as $node)
  {
    ...
    foreach($node->childNodes as $child)
    {
      ...
      if($child->hasChildNodes() && $child->childNodes->item(0)->nodeName == "#cdata-section")
      {
        $text_entries[$text_id][$child->nodeName] = trim($child->childNodes->item(0)->nodeValue);
        $text_entries[$text_id]["#cdata"] = 1;
      }
      else
      {
  ...
  foreach($nodes as $node)
  {
    ...
    $language_entries[$text_id] = [];

    foreach($node->childNodes as $child)
    {
      if($child->nodeName == '#text') continue;
      
      $language_entries[$text_id][$child->nodeName] = trim($child->nodeValue);
    }
  }
  ...
  foreach($nodes as $node)
  {
    ...
    $country_entries[$text_id] = [];

    foreach($node->childNodes as $child)
    {
      if($child->nodeName == '#text') continue;
      
      $country_entries[$text_id][$child->nodeName] = trim($child->nodeValue);
    }
  }
...
function error_class($name)
{
  ...
  if($error_field == $name) echo "error_field";
  else                        echo "";
} // error_class
//-----------------------------------------------------------------
function save_data()
{
  ...
  $document = $xmldoc->appendChild($xmldoc->createElement("document"));
  
  $interface_languages = $document->appendChild($xmldoc->createElement("interface_languages"));
  ...
  foreach($supported_languages as $lang)
  {
    ...
    $interface_language = $xmldoc->createElement("language");
    $interface_language->setAttribute("id", $lang);
    $interface_languages->appendChild($interface_language);
  }
  ...
  if(!empty($_REQUEST["new_language"]["code"]))
  {
    ...
    $interface_language = $xmldoc->createElement("language");
    $interface_language->setAttribute("id", $_REQUEST["new_language"]["code"]);
    $interface_languages->appendChild($interface_language);
  ...
  $languages = $document->appendChild($xmldoc->createElement("languages"));
  
  if(!empty($_REQUEST["new_language_entry"]["code"]))
  {
    $language = $xmldoc->createElement("language");
    $language->setAttribute("id", $_REQUEST["new_language_entry"]["code"]);
    $language = $languages->appendChild($language);
    ...
    foreach($supported_languages as $lang)
    {
      ...
      $lngnode = $xmldoc->createElement($lang);
      $lngnode = $language->appendChild($lngnode);
      
      $txtnode = $xmldoc->createTextNode(checkempty($_REQUEST["new_language_entry"]["langs"][$lang]));
      $lngnode->appendChild($txtnode);
    }
    
    if(!empty($new_lang))
    {
      $lngnode = $xmldoc->createElement($new_lang);
      $lngnode = $language->appendChild($lngnode);
      
      if(!empty($new_lang_source))
      {
        $txtnode = $xmldoc->createTextNode(checkempty($_REQUEST["new_language_entry"]["langs"][$new_lang_source]));
        $lngnode->appendChild($txtnode);
      }
    }
  }
  ...
  foreach($language_entries as $code => $entry_data)
  {
    ...
    foreach($supported_languages as $lang)
    {
      ...
      $txtnode = $xmldoc->createTextNode($entry_data[$lang]);
      $lngnode->appendChild($txtnode);
    }
    ...
    if(!empty($new_lang))
    {
      ...
      if(!empty($new_lang_source))
      {
        $txtnode = $xmldoc->createlanguageNode(checkempty($entry_data[$new_lang_source]));
        $lngnode->appendChild($txtnode);
      }
    }
  ...
  $countries = $document->appendChild($xmldoc->createElement("countries"));
  
  if(!empty($_REQUEST["new_country_entry"]["code"]))
  {
    $country = $xmldoc->createElement("country");
    $country->setAttribute("id", $_REQUEST["new_country_entry"]["code"]);
    ...
    foreach($supported_languages as $lang)
    {
      ...
      $lngnode = $xmldoc->createElement($lang);
      $lngnode = $country->appendChild($lngnode);
      
      $txtnode = $xmldoc->createTextNode(checkempty($_REQUEST["new_country_entry"]["langs"][$lang]));
      $lngnode->appendChild($txtnode);
    }
    ...
    if(!empty($new_lang))
    {
      ...
      if(!empty($new_lang_source))
      {
        $txtnode = $xmldoc->createTextNode(checkempty($_REQUEST["new_country_entry"]["langs"][$new_lang_source]));
        $lngnode->appendChild($txtnode);
      }
    }
  }
  ...
  foreach($country_entries as $code => $entry_data)
  {
    ...
    if(!empty($new_lang))
    {
      ...
      if(!empty($new_lang_source))
      {
        $txtnode = $xmldoc->createcountryNode(checkempty($entry_data[$new_lang_source]));
        $lngnode->appendChild($txtnode);
      }
    }
  ...
  $texts = $document->appendChild($xmldoc->createElement("texts"));
  
  if(!empty($_REQUEST["new_text_entry"]["code"]))
  {
    $text = $xmldoc->createElement("text");
    $text->setAttribute("id", $_REQUEST["new_text_entry"]["code"]);
    $text = $texts->appendChild($text);
    ...
    foreach($supported_languages as $lang)
    {
      ...
      $lngnode = $xmldoc->createElement($lang);
      $lngnode = $text->appendChild($lngnode);
      
      if(!empty($_REQUEST["new_text_entry"]["#cdata"]))
        $txtnode = $xmldoc->createCDATASection(checkempty($_REQUEST["new_text_entry"]["langs"][$lang]));
      else
        $txtnode = $xmldoc->createTextNode(checkempty($_REQUEST["new_text_entry"]["langs"][$lang]));
    ...
    if(!empty($new_lang))
    {
      ...
      {
        if(!empty($_REQUEST["new_text_entry"]["#cdata"]))
          $txtnode = $xmldoc->createCDATASection(checkempty($_REQUEST["new_text_entry"]["langs"][$new_lang_source]));
        else
          $txtnode = $xmldoc->createTextNode(checkempty($_REQUEST["new_text_entry"]["langs"][$new_lang_source]));
  ...
  foreach($text_entries as $code => $entry_data)
  {
    ...
    foreach($supported_languages as $lang)
    {
      ...
      if(!empty($_REQUEST["text_entries"][$counter]["#cdata"]))
        $txtnode = $xmldoc->createCDATASection($entry_data[$lang]);
      else
        $txtnode = $xmldoc->createTextNode($entry_data[$lang]);
    ...
    if(!empty($new_lang))
    {
      ...
      {
        if(!empty($_REQUEST["text_entries"][$counter]["#cdata"]))
          $txtnode = $xmldoc->createCDATASection(checkempty($entry_data[$new_lang_source]));
        else
          $txtnode = $xmldoc->createTextNode(checkempty($entry_data[$new_lang_source]));
  ...
  @copy("texts.xml", "texts_bak.xml");
  
  if(!@$xmldoc->save("texts.xml"))
  {
    $messages .= "<p style='color:red; font-weight: bold'>Translation file 'localization/texts.xml' cannot be saved!</p>";
    return false;
  }
...
{
  $appendix = "";
  if(!empty($_REQUEST["tab"])) $appendix = "?tab=" . $_REQUEST["tab"];
...
<tr>
  ...
  $caption = $lang;
  if(!empty($language_entries[$lang]["en"])) $caption = $language_entries[$lang]["en"] . " [" . $lang . "]";
  ?>
...
<tr>
  ...
  <?php
  $class = "";
  if($error_field == "new_text_entry[code]") $class = "error_field";
  ?>
  ...
  <?php
  $checked = empty($_REQUEST["new_text_entry"]["#cdata"]) ? "" : "checked";
  ?>
  <input type="checkbox" id="new_text_entry_cdata" name="new_text_entry[#cdata]" value="1" onchange="check_changes(this, false)" <?php echo($checked); ?>> <label for="new_text_entry_cdata">CDATA</label>
  </td>

  <?php 
  foreach($supported_languages as $lang): 
  $class = "";
  if($error_field == "new_text_entry[langs][$lang]") $class = "error_field";
  ?>
...
<tr>
  ...
  <?php
  $class = "";
  if(empty($_REQUEST["text_entries"][$counter]["code"])) $class = "empty_field";
  if($error_field == "text_entries[$counter][code]") $class = "error_field";
  ?>
  ...
  <?php else: 
  $class = "";
  if(empty($code)) $class = "empty_field";
  ?>
  ...
  <?php
  $checked = empty($_REQUEST["text_entries"][$counter]["#cdata"]) ? "" : "checked";
  ?>
  <input type="checkbox" id="text_entries_<?php echo($counter); ?>_cdata" name="text_entries[<?php echo($counter); ?>][#cdata]" value="1" onchange="check_changes(this, false)" <?php echo($checked); ?>> <label for="text_entries_<?php echo($counter); ?>_cdata">CDATA</label> 
   </td>

  <?php 
  foreach($supported_languages as $lang): 
  $class = "";
  if(empty($_REQUEST["text_entries"][$counter]["langs"][$lang])) $class = "empty_field";
  if($error_field == "text_entries[$counter][langs][$lang]") $class = "error_field";
  ...
  <?php else: 
  $class = "";
  if(empty($entry_data[$lang])) $class = "empty_field";
  ?>
...
<tr>
  ...
  <?php
  $class = "";
  if($error_field == "new_language_entry[code]") $class = "error_field";
  ?>
  ...
  foreach($supported_languages as $lang): 
  $class = "";
  if($error_field == "new_language_entry[langs][$lang]") $class = "error_field";
  ?>
...
<tr>
  ...
  <?php
  $class = "";
  if(empty($_REQUEST["language_entries"][$counter]["code"])) $class = "empty_field";
  if($error_field == "language_entries[$counter][code]") $class = "error_field";
  ?>
  ...
  foreach($supported_languages as $lang): 
  $class = "";
  if(empty($_REQUEST["language_entries"][$counter]["langs"][$lang])) $class = "empty_field";
  if($error_field == "language_entries[$counter][langs][$lang]") $class = "error_field";
  ?>
  <td>
...
<tr>
  ...
  <?php
  $class = "";
  if($error_field == "new_country_entry[code]") $class = "error_field";
  ?>
  ...
  foreach($supported_languages as $lang): 
  $class = "";
  if($error_field == "new_country_entry[langs][$lang]") $class = "error_field";
  ?>
...
<tr>
  ...
  <?php
  $class = "";
  if(empty($_REQUEST["country_entries"][$counter]["code"])) $class = "empty_field";
  if($error_field == "country_entries[$counter][code]") $class = "error_field";
  ?>
  ...
  foreach($supported_languages as $lang): 
  $class = "";
  if(empty($_REQUEST["country_entries"][$counter]["langs"][$lang])) $class = "empty_field";
  if($error_field == "country_entries[$counter][code]") $class = "error_field";
  ?>
  <td>
